...
class miio_memcpy(angr.SimProcedure):
    def run(self):
        ...
        operations="<Operation {0} ({1}, {2}, {3})>".format("memcpy",hex(self.state.se.eval(self.state.regs.r0)),hex(self.state.se.eval(self.state.regs.r1)),self.state.se.eval(self.state.regs.r2))
        print operations
